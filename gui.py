...
def select_file():
    ...
    if filepath:
        ...
        select_file_button.pack_forget()  # Hide the "Select File" button
        progress_bar.pack(pady=(10, 0))  # Show the progress bar
        progress_bar.start(10)  # Start the progress bar when file is selected
        status_label.config(text="Please wait...")  # Show the status message
